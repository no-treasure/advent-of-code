  ...
  return lines.reduce((acc, line, lineIndex) => {
    return (
      ...
      line.split('').reduce((wordAcc, word, wordIndex) => {
        ...
        const isHorizontal =
          wordIndex + 3 <= line.length && line.slice(wordIndex, wordIndex + 4) === 'XMAS'

        const isBackwardHorizontal =
          wordIndex - 3 >= 0 && line.slice(wordIndex - 3, wordIndex + 1) === 'SAMX'
        ...
        const isBackwardVertical =
          lineIndex - 3 >= 0 &&
          lines[lineIndex - 1][wordIndex] === 'M' &&
          lines[lineIndex - 2][wordIndex] === 'A' &&
          lines[lineIndex - 3][wordIndex] === 'S'

        const isRightBottomDiagonal =
          lineIndex + 3 < lines.length &&
